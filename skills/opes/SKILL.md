---
name: opes
description: opeskill 超级个体工具箱总入口。用于商业化、offer、定价、获客、销售、一人公司、创作者商业、个人品牌、内容、领导力、运营、招聘、财富判断、杠杆、健康、睡眠、专注、拖延、行动改变、自信、交易系统和风险管理。普通问题默认使用本地专家原子库和 Skill 知识包；只有用户主动调用对应 ima 入口或明确要求 IMA 检索/引用/排错时，才检索 IMA。安装 SkillHub 轻量包后，可调用 $opes-download-atoms 下载、解压并安装全量 7 人专家原子库；商业案例库使用共享依赖 $commercial-case-library。
---
...
SkillHub 上传包是轻量包，不直接内置全量大原子库。对于需要资料依据的回答，默认使用本地专家原子库和 `知识库/Skill知识包/` 方法说明。只有用户主动调用对应 `ima` 入口，或明确要求“检索 IMA / 引用 IMA / 排查 IMA / 从 IMA 找资料”时，才使用 IMA。
...
不要把 IMA 作为默认来源。普通请求使用已经安装的本地原子库和 `知识库/Skill知识包/` 方法说明。只要回答需要本地原子库且原子库缺失，就自动调用 `$opes-download-atoms` 或运行下载脚本，不要询问用户是否下载。

## IMA 检索入口

普通请求不默认检索 IMA。只有用户主动要求 IMA 检索、引用、排错，或明确指定 IMA 知识库时，才使用这些名称作为参考：

| skill | 显式 IMA 请求时使用的知识库 |
| --- | --- |
...
如果 IMA 知识库没有找到，不要停止任务。说明 IMA 未命中，然后继续使用对应本地 `知识库/原子库/atoms.jsonl`。
...
6. 每个已选子 skill 必须遵守自己的检索规则：
   - 普通请求默认使用打包的 `知识库/原子库/atoms.jsonl` 和 `知识库/Skill知识包/`；
   - 只有用户显式要求 IMA 检索/引用/排错时，才尝试对应 IMA 知识库；
   - IMA 失败或没有有效命中时，继续回到本地原子库和方法说明。
...
1. 使用每个已选子 skill 自己的“本地优先、显式 IMA 才检索”规则。
2. 默认不要检索全部七个模块。
3. 在把答案标为“有资料依据”之前，至少使用主模块的一条相关来源。普通请求优先使用本地原子库和方法说明；只有显式 IMA 请求才使用 IMA 证据。
...
1. 根据用户问题类型选择主专家模块。
2. 使用该模块的本地专家原子或方法说明形成诊断和建议；只有显式 IMA 请求才加入 IMA 证据。
